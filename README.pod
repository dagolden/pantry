...
such as "test", "staging" or "production".  Any node command can
be given an environment selector with the C<--env ENV_NAME> or
C<-E ENV_NAME> options and all operations will be restricted to that
environment name.  This even works with "list" for a handy way to sync
all nodes:
...
better documentation/constraints for environments
